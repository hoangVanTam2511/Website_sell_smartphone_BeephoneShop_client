...
const CartPage = (props) => {
  ...
  const getBillAfterBuy = async(id_bill, status) => {
    ...
    await request('GET',`/client/bill/get-order-after-buy?id_bill=${id_bill}`).then(
      (response) => {
        if(response.status === 200) {
          console.log(response.data)
          setBill(response.data);
          if(status == "01"){
            request('PUT',`/client/bill/update-state-bill?id_bill=${response.data.id}`).then((response) => {
          ...
          >
            ...
            >
              ...
              >
                ...
                {!isLoading && state === "00" ? (
                  <img
                    style={{ width: "35px", height: "35px", margin: '0 auto', marginBottom: '8px' }}
                    src="https://icons.veryicon.com/png/o/miscellaneous/8atour/success-35.png"
                  />
                ) : !isLoading && state === "01" ? (
                  <img
                ...
                >
                  ...
                  {bill.totalPrice > bill.totalPriceAfterPrice ? (
                    ...
                    >
                      ...
                      <div style={{ color: "red" }}>
                        - {formatMoney(bill.totalPrice - bill.totalPriceAfterPrice)}
                      </div>
                    </div>
                  ):(<></>)}
