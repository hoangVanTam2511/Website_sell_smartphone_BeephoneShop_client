...
const CartPage = props => {
  ...
  const getAllDistrictGhnByIdProvinceByAddressCustomer = async (provinceId, type, data) => {
    await axios
      .get(
  ...
  const getAllWardGhnByIdDistrictByAddressCustonmer = async (districtId, type, data) => {
    await axios
      .get(
    ...
    axios
      ...
      .then(response => {
        ...
        if(props.informationBill.province !== ''){ 
          var provinceId = response.data.data.find(e => e.ProvinceName === props.informationBill.province)
          if(provinceId !== undefined){
            setProvinceSelected(provinceId.ProvinceID)
    ...
    await axios
      ...
      .then(response => {
        ...
        if(props.informationBill.district !== '' && type === 1 ){
          var district_id = response.data.data.find(e => e.DistrictName === props.informationBill.district)
          if(district_id !== undefined){
            setDistrictSelected(district_id.DistrictID)
    ...
    await axios
      ...
      .then(response => {
        ...
        if(props.informationBill.ward !== '' && type === 1){
          var ward_id = response.data.data.find(e => e.WardName === props.informationBill.ward)
          if(ward_id !== undefined){
            setWardSelected(ward_id.WardCode)
  ...
  const checkValidate = data => {
    ...
    if (flag === 0) {
      var data = {
        ...
        district: districts.find(item => item.DistrictID === districtSelected).DistrictName,
        province: provinces.find(item => item.ProvinceID === provinceSelected).ProvinceName,
        shipFee: shipFee,
                ...
                {shipFee > 0 && receivedDate !== '' ? (
                  <>
                    ...
                    >
                      ...
                      <div style={{ style: '#707070' }}>{formatMoney(shipFee)}</div>
                    </div>
